...
function loadRuntime(){
  ...
  const refs=[...index.matchAll(/<script src="([^"]+)"><\/script>/g)]
    .map(m=>m[1])
    .filter(x=>/^games\/(?:sudoku-bank(?:-iteration\d+)?|sudoku-generator|iteration7-generator-hardening)\.js$/.test(x));
...
function topology(generated){
  return canonicalJson((generated.data&&generated.data.battenburg)||[]);
}
...
const jobs=Array.from({length:samples},(_,i)=>({seed:(0x62617400+i*0x1f3d)>>>0}));
const rows=[];
...
console.log('BATTENBURG_PRODUCTION_GATE '+JSON.stringify({samples,perSampleTimeoutMs,concurrency,report}));
console.log('BATTENBURG_PRODUCTION_GATE:'+(pass?'PASS':'FAIL'));
