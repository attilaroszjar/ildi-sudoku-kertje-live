...
if(typeof generator.countDoubleSkyscraperSolutions!=='function')throw new Error('countDoubleSkyscraperSolutions is unavailable');
...
if(!generated||!Array.isArray(generated.puzzle))throw new Error('Double Skyscraper production generator returned no puzzle');

function countVariant(grid,ignoreSpecial=false){return generator.countDoubleSkyscraperSolutions(grid,generated,2,ignoreSpecial);}
...
console.log(`DOUBLE_SKYSCRAPER_EXPERT_FRONTIER seed=${seed} difficulty=${difficulty} hostRealm=true`);
console.log(`START givens=${givens}/${total} density=${(givens/total).toFixed(3)} generationMs=${round1(generationMs).toFixed(1)}`);
console.log(`BASELINE variantSolutions=${baseline} baseSolutions=${baseWithoutSpecial} variantEssential=${baseline===1&&baseWithoutSpecial!==1} runtimeMs=${round1(baselineMs).toFixed(1)} counter=countDoubleSkyscraperSolutions`);
console.log(`STRUCTURE outsideClues=${outsideClues}`);
...
if(baseWithoutSpecial===1)failures.push('baseline is not variant-essential without Double Skyscraper special rules');
...
if(failures.length){for(const failure of failures)console.error(`DOUBLE_SKYSCRAPER_EXPERT_FRONTIER_FAILURE ${failure}`);console.log('DOUBLE_SKYSCRAPER_EXPERT_FRONTIER:FAIL');process.exitCode=1;}
else console.log('DOUBLE_SKYSCRAPER_EXPERT_FRONTIER:PASS');
