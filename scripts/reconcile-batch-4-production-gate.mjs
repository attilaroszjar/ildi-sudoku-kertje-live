...
const ids=['anti-king','anti-knight','nonconsecutive'];
...
function loadRuntime(){
  ...
  const refs=[...index.matchAll(/<script src="([^"]+)"><\/script>/g)]
    .map(m=>m[1])
    .filter(x=>/^games\/(?:sudoku-bank(?:-iteration\d+)?|sudoku-generator|iteration4-generator-hardening)\.js$/.test(x));
  ...
  if(typeof ctx.SudokuGenerator?.makeSeededAntiSolution!=='function')throw new Error('Batch 4 runtime bootstrap did not load anti-constraint production hardening');
  if(typeof ctx.SudokuGenerator?.makeSeededNonconsecutiveSolution!=='function')throw new Error('Batch 4 runtime bootstrap did not load bounded Non-Consecutive construction');
  return ctx;
}
...
if(process.argv[2]==='--worker'){
  ...
  try{
    ...
    const constructionOk=id!=='nonconsecutive'||flags.solutionConstruction==='nonconsecutive-algebraic-template';
    const contractOk=exact===1&&classic>1&&replayOk&&flags.unique===true&&flags.variantEssential===true&&flags.generatorFamily==='anti-constraint-seeded-full-solution'&&constructionOk;
    ...
    const exhausted=/GENERATION_EXHAUSTED|generation exhausted|bounded generation failed|generation failed|generation exhausted/i.test(message);
...
const jobs=[];
for(let vi=0;vi<ids.length;vi++)for(let si=0;si<samplesPerVariant;si++)jobs.push({id:ids[vi],seed:(0x6b440000+vi*0x10000+si*0x1f3d)>>>0});
const rows=[];
...
for(const id of ids){
  ...
  const result=summarizeProductionSamples(evaluated,{expectedSamples:samplesPerVariant,requireTopology:false,maxSampleMs:perSampleTimeoutMs});
  const failureSamples=vr.filter(r=>r.status!=='PASS').map(r=>({seed:r.seed,status:r.status,error:r.error,exact:r.exact,classic:r.classic,replayOk:r.replayOk,generation:r.generation}));
  byVariant[id]={...result.summary,exhausted:exhausted.length,failureReasons:result.failures,failureSamples};
  if(!result.pass)failures.push(id);
}
const pass=failures.length===0;
console.log('RECONCILE_BATCH_4_PRODUCTION_GATE '+JSON.stringify({samplesPerVariant,perSampleTimeoutMs,concurrency,byVariant,failures}));
console.log('RECONCILE_BATCH_4_PRODUCTION_GATE:'+(pass?'PASS':'FAIL'));
