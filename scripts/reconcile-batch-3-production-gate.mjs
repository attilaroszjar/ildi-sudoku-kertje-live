...
function loadRuntime(){
  ...
  const refs=[...index.matchAll(/<script src="([^"]+)"><\/script>/g)]
    .map(m=>m[1])
    .filter(x=>/^games\/(?:sudoku-bank(?:-iteration\d+)?|extra-house-generator-core|jigsaw-generator-hardening|x-sums-runtime-hardening|sudoku-generator)\.js$/.test(x));
...
function topologyFingerprint(id,generated){
  ...
  if(id==='sandwich')return canonicalJson(data.clues||[]);
  if(id==='jigsaw')return canonicalJson(data.regions||[]);
  throw new Error('missing topology adapter for '+id);
}

function exactSolutionCount(id,G,generated){
  if(id==='jigsaw')return G.countJigsawSolutions(generated.puzzle,generated.data&&generated.data.regions,2);
...
if(process.argv[2]==='--worker'){
  ...
  try{
    ...
    const exact=exactSolutionCount(id,G,generated);
    ...
    console.log(JSON.stringify({
      ...
      topology:topologyFingerprint(id,generated),
...
const jobs=[];
for(let vi=0;vi<ids.length;vi++)for(let si=0;si<samplesPerVariant;si++)jobs.push({id:ids[vi],seed:(0x6b430000+vi*0x10000+si*0x1f3d)>>>0});
const rows=[];
...
for(const id of ids){
  ...
  const failureSamples=vr.filter(r=>r.status!=='PASS').map(r=>({seed:r.seed,status:r.status,exact:r.exact,classic:r.classic,replayOk:r.replayOk,error:r.error}));
...
console.log('RECONCILE_BATCH_3_PRODUCTION_GATE '+JSON.stringify({samplesPerVariant,perSampleTimeoutMs,concurrency,byVariant,failures}));
console.log('RECONCILE_BATCH_3_PRODUCTION_GATE:'+(pass?'PASS':'FAIL'));
