...
const variant=globalThis.SudokuBank.find(entry=>entry&&entry.id==='mini');
if(!variant)throw new Error('mini not found in production SudokuBank');

const generationStart=performance.now();
const generated=globalThis.SudokuGenerator.make(variant,seed,difficulty);
const generationMs=performance.now()-generationStart;
if(!generated||!Array.isArray(generated.puzzle))throw new Error('Mini production generator returned no puzzle');
...
console.log(`MINI4_EXPERT_FRONTIER seed=${seed} difficulty=${difficulty} hostRealm=true`);
...
const slowest=checks.slice().sort((a,b)=>b.elapsedMs-a.elapsedMs).slice(0,8);
...
if(generated.generation?.verification!=='solver-verified-local-irreducible')failures.push(`generation.verification is ${generated.generation?.verification}, expected solver-verified-local-irreducible`);
if(removable.length!==0)failures.push(`${removable.length} remaining givens are individually removable`);

if(failures.length){
  for(const failure of failures)console.error(`MINI4_EXPERT_FRONTIER_FAILURE ${failure}`);
  console.log('MINI4_EXPERT_FRONTIER:FAIL');
  process.exitCode=1;
}else{
  console.log('MINI4_EXPERT_FRONTIER:PASS');
}
