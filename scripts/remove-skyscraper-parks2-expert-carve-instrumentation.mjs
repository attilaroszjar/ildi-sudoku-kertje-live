...
if(!text.includes('PARKS2_CARVE_BEGIN')){
  console.log('SKYSCRAPER_PARKS2_EXPERT_CARVE_INSTRUMENTATION_CLEANUP:PASS already-clean=true');
  process.exit(0);
}

const instrumented=`      for(oi=0;oi<order.length;oi+=1){
...
const clean=`      for(oi=0;oi<order.length;oi+=1){
        idx=order[oi];r=Math.floor(idx/n);c=idx%n;old=grid[r][c];if(!old)continue;
        grid[r][c]=0;
...
if(!text.includes(instrumented))throw new Error('Parks2 instrumented carve loop anchor not found');
text=text.replace(instrumented,clean);
fs.writeFileSync(file,text);
console.log('SKYSCRAPER_PARKS2_EXPERT_CARVE_INSTRUMENTATION_CLEANUP:PASS already-clean=false');
