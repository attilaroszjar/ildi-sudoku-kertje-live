...
if(typeof generator.countEvenOddSkyscraperSolutions!=='function')throw new Error('countEvenOddSkyscraperSolutions is unavailable');
...
if(!generated||!Array.isArray(generated.puzzle))throw new Error('Even/Odd Skyscraper production generator returned no puzzle');

function countVariant(grid,ignoreSpecial=false){
  return generator.countEvenOddSkyscraperSolutions(grid,generated,2,ignoreSpecial);
}
...
const parityCells=Array.isArray(generated.data?.parityCells)?generated.data.parityCells.length:0;

console.log(`EVENODD_SKYSCRAPER_EXPERT_FRONTIER seed=${seed} difficulty=${difficulty} hostRealm=true`);
console.log(`START givens=${givens}/${total} density=${(givens/total).toFixed(3)} generationMs=${round1(generationMs).toFixed(1)}`);
console.log(`BASELINE variantSolutions=${baseline} baseSolutions=${baseWithoutSpecial} variantEssential=${baseline===1&&baseWithoutSpecial!==1} runtimeMs=${round1(baselineMs).toFixed(1)} counter=countEvenOddSkyscraperSolutions`);
console.log(`STRUCTURE outsideClues=${outsideClues} parityCells=${parityCells}`);
...
for(let r=0;r<generated.puzzle.length;r++){
  for(let c=0;c<generated.puzzle[r].length;c++){
    ...
    if(removableByUniqueness){
      baseSolutions=countVariant(candidate,true);
      variantEssential=baseSolutions!==1;
    }
    checks.push({
      row:r+1,col:c+1,value,variantSolutions,baseSolutions,variantEssential,
      removable:removableByUniqueness&&variantEssential!==false,
      elapsedMs
    });
  }
}
...
if(baseWithoutSpecial===1)failures.push('baseline is not variant-essential without Even/Odd Skyscraper special rules');
...
if(parityCells===0)failures.push('parity cell structure is empty');
...
if(!String(generated.generation?.generatorFamily||'').startsWith('seeded-row-column-permutation-expert-local-irreducible'))failures.push(`generatorFamily=${generated.generation?.generatorFamily}, expected expert local-irreducible family`);
...
if(failures.length){
  for(const failure of failures)console.error(`EVENODD_SKYSCRAPER_EXPERT_FRONTIER_FAILURE ${failure}`);
  console.log('EVENODD_SKYSCRAPER_EXPERT_FRONTIER:FAIL');
  process.exitCode=1;
}else{
  console.log('EVENODD_SKYSCRAPER_EXPERT_FRONTIER:PASS');
}
