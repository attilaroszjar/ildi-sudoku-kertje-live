...
function loadRuntime(){
  ...
  const refs=[...index.matchAll(/<script src="([^"]+)"><\/script>/g)].map(m=>m[1]).filter(x=>/^games\/(?:sudoku-bank(?:-iteration\d+)?|sudoku-generator|iteration13-generator-hardening)\.js$/.test(x));
  ...
  const G=ctx.SudokuGenerator,variant=ctx.SudokuBank&&ctx.SudokuBank.find(v=>v.id==='even-sandwich');
  if(!G||typeof G.make!=='function')throw new Error('SudokuGenerator.make missing');
  if(!variant)throw new Error('Even Sandwich missing');
  return {G,variant};
}
...
for(let i=0;i<samplesPerDifficulty;i++){
  const seed=(0x65736400+i*0x1f3d)>>>0;
...
console.log('EVEN_SANDWICH_DIFFICULTY_GATE '+JSON.stringify({samplesPerDifficulty,timeoutMs,concurrency,byDifficulty,orderingBasis:'paired-givens-pressure',pairedClueOrder,failures:rows.filter(r=>r.status!=='PASS')}));
console.log('EVEN_SANDWICH_DIFFICULTY_GATE:'+(pass?'PASS':'FAIL'));
