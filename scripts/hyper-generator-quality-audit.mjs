...
vm.runInContext(hardening,ctx,{filename:'extra-house-generator-hardening.js'});

const G=ctx.SudokuGenerator;
const variant=ctx.SudokuBank.find(v=>v.id==='hyper');
if(!variant)throw new Error('canonical Hyper variant missing');
...
const seeds=Array.from({length:32},(_,i)=>7000+i);
...
for(const difficulty of difficulties){
  ...
  for(const seed of seeds){
    ...
    const variantOk=g.generation&&g.generation.unique===true&&G.countVariantSolutions(g.puzzle,g,2)===1;
...
console.log('HYPER_GENERATOR_AUDIT '+JSON.stringify({
  seeds:seeds.length,
  generated:total,
  exactUnique,
  variantEssential,
...
console.log('HYPER_GENERATOR_AUDIT:'+(pass?'PASS':'FAIL'));
