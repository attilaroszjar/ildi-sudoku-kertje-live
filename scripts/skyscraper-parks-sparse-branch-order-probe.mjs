...
function countOrdered(source,limit=2,mode='solution-first'){
  const grid=source.map(r=>r.slice()),rows=Array(n).fill(0),cols=Array(n).fill(0);
  const stats={nodes:0,branches:0,deadEnds:0,solutions:0,assignmentsTried:0,boundPrunes:0,maxDepth:0,firstSolutionNode:null,secondSolutionNode:null};
  ...
  function orderedBits(mask,r,c){
    const out=[];
    if(mode==='solution-first'){
      const d=generated.solution[r][c],one=1<<(d-1);
      if(mask&one)out.push(one);
    }
    for(let bits=mask;bits;bits&=bits-1){const one=bits&-bits;if(!out.includes(one))out.push(one);}
    if(mode==='descending')out.sort((a,b)=>b-a);
    return out;
  }
  function visit(depth){
    ...
    outer:for(let r=0;r<n;r++)for(let c=0;c<n;c++)if(!grid[r][c]){
      ...
      if(cnt<best){br=r;bc=c;bm=mask;best=cnt;if(cnt===1)break outer;}
    }
    if(br<0){
      stats.solutions++;
      if(stats.solutions===1)stats.firstSolutionNode=stats.nodes;
      if(stats.solutions===2)stats.secondSolutionNode=stats.nodes;
      return;
    }
    if(best>1)stats.branches++;
    for(const one of orderedBits(bm,br,bc)){
      const d=1+Math.round(Math.log(one)/Math.LN2);stats.assignmentsTried++;
...
let mismatches=0,productionMs=0,orderedMs=0;
for(const tc of cases){
  const t0=performance.now(),prod=G.countParkSolutions(tc.puzzle,generated,2,false);productionMs+=performance.now()-t0;
  const opt=countOrdered(tc.puzzle,2,'solution-first');orderedMs+=opt.runtimeMs;if(prod!==opt.solutions)mismatches++;
}
console.log('SKYSCRAPER_PARKS_BRANCH_ORDER_DIFFERENTIAL '+JSON.stringify({seed,cases:cases.length,mismatches,productionMs:+productionMs.toFixed(1),solutionFirstMs:+orderedMs.toFixed(1),aggregateSpeedup:orderedMs?+(productionMs/orderedMs).toFixed(2):null}));
if(mismatches){console.log('SKYSCRAPER_PARKS_BRANCH_ORDER_GATE:FAIL');process.exitCode=1;}else{
  ...
  console.log('SKYSCRAPER_PARKS_BRANCH_ORDER_PROGRESS '+JSON.stringify({phase:'hotspot-start',cell:'r7c6',cluesBefore,trialClues:12,mode:'solution-first'}));
  const result=countOrdered(trial,2,'solution-first');
  let baseFamilySolutions=null;
  if(result.solutions===1)baseFamilySolutions=G.countParkSolutions(trial,generated,2,true);
  console.log('SKYSCRAPER_PARKS_BRANCH_ORDER_HOTSPOT '+JSON.stringify({seed,cell:'r7c6',value:8,cluesBefore,trialClues:12,mode:'solution-first',solutions:result.solutions,baseFamilySolutions,productionContractPass:result.solutions===1&&baseFamilySolutions>1,runtimeMs:+result.runtimeMs.toFixed(1),stats:result.stats}));
  console.log('SKYSCRAPER_PARKS_BRANCH_ORDER_GATE:PASS');
}
