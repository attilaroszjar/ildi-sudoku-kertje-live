...
const variant=globalThis.SudokuBank.find(entry=>entry&&entry.id==='mini-6');
if(!variant)throw new Error('mini-6 not found in production SudokuBank');

const generationStart=performance.now();
const generated=globalThis.SudokuGenerator.make(variant,seed,difficulty);
const generationMs=performance.now()-generationStart;
if(!generated||!Array.isArray(generated.puzzle))throw new Error('Mini-6 production generator returned no puzzle');
...
console.log(`MINI6_EXPERT_FRONTIER seed=${seed} difficulty=${difficulty} hostRealm=true`);
...
if(failures.length){
  for(const failure of failures)console.error(`MINI6_EXPERT_FRONTIER_FAILURE ${failure}`);
  console.log('MINI6_EXPERT_FRONTIER:FAIL');
  process.exitCode=1;
}else{
  console.log('MINI6_EXPERT_FRONTIER:PASS');
}
