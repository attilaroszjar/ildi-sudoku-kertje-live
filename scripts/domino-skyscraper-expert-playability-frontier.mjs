...
if(!generated||!Array.isArray(generated.puzzle))throw new Error('Domino Skyscraper production generator returned no puzzle');
...
const dominoes=Array.isArray(generated.data?.dominoes)?generated.data.dominoes.length:0;

console.log(`DOMINO_SKYSCRAPER_EXPERT_FRONTIER seed=${seed} difficulty=${difficulty} hostRealm=true`);
console.log(`START givens=${givens}/${total} density=${(givens/total).toFixed(3)} generationMs=${round1(generationMs).toFixed(1)}`);
console.log(`BASELINE variantSolutions=${baseline} baseSolutions=${baseWithoutSpecial} variantEssential=${baseline===1&&baseWithoutSpecial!==1} runtimeMs=${round1(baselineMs).toFixed(1)} counter=countDominoSkyscraperSolutions`);
console.log(`STRUCTURE outsideClues=${outsideClues} dominoes=${dominoes}`);
...
if(baseWithoutSpecial===1)failures.push('baseline is not variant-essential without Domino Skyscraper special rules');
...
if(dominoes===0)failures.push('domino structure is empty');
...
if(failures.length){
  for(const failure of failures)console.error(`DOMINO_SKYSCRAPER_EXPERT_FRONTIER_FAILURE ${failure}`);
  console.log('DOMINO_SKYSCRAPER_EXPERT_FRONTIER:FAIL');
  process.exitCode=1;
}else{
  console.log('DOMINO_SKYSCRAPER_EXPERT_FRONTIER:PASS');
}
