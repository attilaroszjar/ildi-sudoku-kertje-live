...
const ids=['ascending-sequences','running-cells'];
...
function clueCount(puzzle){
  return puzzle.flat().filter(Boolean).length;
}
...
for(const id of ids){
  ...
  for(let i=0;i<samples;i++){
    const seed=(0x4f440000+ids.indexOf(id)*0x10000+i*0x1f3d)>>>0;
  ...
  for(const difficulty of difficulties){
    ...
    byDifficulty[difficulty]={samples:rs.length,passes:rs.filter(r=>r.status==='PASS').length,timeouts:rs.filter(r=>r.status==='TIMEOUT').length,failures:rs.filter(r=>!['PASS','TIMEOUT'].includes(r.status)).length,medianClues:median(rs.filter(r=>r.status==='PASS').map(r=>r.clues)),medianDifficultyScore:median(rs.filter(r=>r.status==='PASS').map(r=>r.score)),maxMs:rs.filter(r=>r.status==='PASS').length?Math.max(...rs.filter(r=>r.status==='PASS').map(r=>r.ms)):null};
  }
...
for(const r of all){
  ...
  console.log(`ORDERING_DIFFICULTY ${r.id} ${r.pass?'PASS':'FAIL'} clues=${g.medianClues}>${f.medianClues}>${e.medianClues} scores=${g.medianDifficultyScore}/${f.medianDifficultyScore}/${e.medianDifficultyScore} maxMs=${Math.max(g.maxMs||0,f.maxMs||0,e.maxMs||0)}`);
}
const pass=all.every(r=>r.pass);
console.log('ORDERING_FAMILY_DIFFICULTY_GATE:'+(pass?'PASS':'FAIL'));
