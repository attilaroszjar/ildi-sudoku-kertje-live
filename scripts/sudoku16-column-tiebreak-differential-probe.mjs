...
function count16Tiebreak(source,limit=2,stats){
  ...
  function impact(c){
    ...
    for(let r=c.D;r!==c;r=r.D){
      let rowScore=0;
      for(let j=r.R;j!==r;j=j.R)rowScore+=j.C.size;
      score+=rowScore;
    }
    return score;
  }
  ...
  function search(){
    ...
    let min=1e9,candidates=[];
    for(let c=rootNode.R;c!==rootNode;c=c.R){
      if(c.size<min){min=c.size;candidates=[c];}
      else if(c.size===min)candidates.push(c);
    }
    let chosen=null;
    if(candidates.length===1)chosen=candidates[0];
      ...
      for(const c of candidates){
        const s=impact(c);
        if(s<bestScore){bestScore=s;chosen=c;}
      }
    }
  ...
  search();
  if(stats)Object.assign(stats,{nodes,branches,deadEnds,solutions,openCells:empties.length,tieBreaks,columnTieBreak:'min-impact-sum'});
  return solutions;
}
...
const tiebreak=carve(count16Tiebreak);
const sameDecisions=baseline.decisions.length===tiebreak.decisions.length&&baseline.decisions.every((x,i)=>x===tiebreak.decisions[i]);
const samePuzzle=JSON.stringify(baseline.grid)===JSON.stringify(tiebreak.grid);
const worst=tiebreak.calls.slice().sort((a,b)=>b.ms-a.ms).slice(0,10);
emit('SUDOKU16_COLUMN_TIEBREAK_DIFFERENTIAL',{
  ...
  tiebreak:{ms:rounded(tiebreak.ms),finalGivens:countGivens(tiebreak.grid),accepted:tiebreak.accepted,rejected:tiebreak.rejected,verificationMs:rounded(tiebreak.verificationMs),finalStats:tiebreak.finalStats,worstCalls:worst.map(x=>({...x,ms:rounded(x.ms)}))},
  sameDecisions,samePuzzle,speedup:rounded(baseline.ms/tiebreak.ms)
});
if(!baseline.unique||!tiebreak.unique||!sameDecisions||!samePuzzle)throw new Error('column tie-break differential mismatch');
console.log('SUDOKU16_COLUMN_TIEBREAK_DIFFERENTIAL:PASS');
