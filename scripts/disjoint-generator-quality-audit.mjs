...
vm.runInContext(bank2,ctx,{filename:'sudoku-bank-iteration2.js'});
...
vm.runInContext(hardening,ctx,{filename:'disjoint-groups-generator-hardening.js'});

const G=ctx.SudokuGenerator;
const variant=ctx.SudokuBank.find(v=>v.id==='disjoint-groups');
if(!variant)throw new Error('canonical Disjoint Groups variant missing');

const difficulties=['gentle','focused','expert'];
const targets={gentle:40,focused:30,expert:27};
const seeds=Array.from({length:32},(_,i)=>9000+i);
...
function disjointValid(grid){for(let rr=0;rr<3;rr++)for(let cc=0;cc<3;cc++){const vals=[];for(let br=0;br<3;br++)for(let bc=0;bc<3;bc++)vals.push(grid[br*3+rr][bc*3+cc]);if(new Set(vals).size!==9)return false;}return true;}

const solutionHashes=new Set(),structuralHashes=new Set(),rows={};
...
for(const difficulty of difficulties){
  ...
  for(const seed of seeds){
    ...
    const variantOk=g.generation&&g.generation.unique===true&&G.countVariantSolutions(g.puzzle,g,2)===1&&disjointValid(g.solution);
  ...
  rows[difficulty]={puzzleUnique:puzzleHashes.size,scoreMedian:median(scores),clueMin:Math.min(...clues),clueMax:Math.max(...clues),runtimeP50Ms:round(percentile(times,0.50)),runtimeP95Ms:round(percentile(times,0.95)),runtimeMaxMs:round(Math.max(...times))};
}
...
const checks=[
  ...
  rows.gentle.runtimeP95Ms<1000,
  rows.focused.runtimeP95Ms<1000,
  rows.expert.runtimeP95Ms<1500,
  failures.length===0
];
const pass=checks.every(Boolean);
console.log('DISJOINT_GENERATOR_AUDIT '+JSON.stringify({seeds:seeds.length,generated:total,exactUnique,variantEssential,solutionUnique:solutionHashes.size,structuralSolutionUnique:structuralHashes.size,difficulties:rows,failures:failures.length}));
console.log('DISJOINT_GENERATOR_AUDIT:'+(pass?'PASS':'FAIL'));
