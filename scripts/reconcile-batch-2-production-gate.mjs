...
const ids=['fortress','x-sums','rossini'];
...
function loadRuntime(){
  ...
  const bankRefs=[...index.matchAll(/<script src="([^"]+)"><\/script>/g)]
    .map(m=>m[1])
    .filter(x=>/^games\/sudoku-bank(?:-iteration\d+)?\.js$/.test(x));
  const files=[...bankRefs,'games/sudoku-generator.js','games/x-sums-runtime-hardening.js'];
...
function topologyFingerprint(id,generated){
  ...
  if(id==='fortress')return canonicalJson(data.cells||[]);
  if(id==='x-sums'||id==='rossini')return canonicalJson(data.clues||[]);
...
if(process.argv[2]==='--worker'){
  ...
  try{
    ...
    const exhausted=/GENERATION_EXHAUSTED|bounded generation failed|generation failed/i.test(message);
...
const jobs=[];
for(let vi=0;vi<ids.length;vi++)for(let si=0;si<samplesPerVariant;si++)jobs.push({id:ids[vi],seed:(0x6b320000+vi*0x10000+si*0x1f3d)>>>0});
const rows=[];
...
console.log('RECONCILE_BATCH_2_PRODUCTION_GATE '+JSON.stringify({samplesPerVariant,perSampleTimeoutMs,concurrency,byVariant,failures}));
console.log('RECONCILE_BATCH_2_PRODUCTION_GATE:'+(pass?'PASS':'FAIL'));
