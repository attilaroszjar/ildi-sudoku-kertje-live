...
function findAlternative(source,nodeBudget){
  ...
  let nodes=0,found=false,exhausted=false,forced=0;
  const box=(r,c)=>Math.floor(r/3)*3+Math.floor(c/3);
  const baseMask=(r,c)=>full&~(rows[r]|cols[c]|boxes[box(r,c)]);
  function clueFeasible(ci){const cl=compiled[ci];let sum=0,min=0,max=0,empty=0;for(const idx of cl.cells){const r=Math.floor(idx/9),c=idx%9,v=grid[r][c];if(v){sum+=v;continue;}const mask=baseMask(r,c);if(!mask)return false;min+=lowDigit[mask];max+=highDigit[mask];empty++;}if(sum>cl.sum)return false;if(!empty)return sum===cl.sum;return sum+min<=cl.sum&&sum+max>=cl.sum;}
  function allowedMask(r,c){let out=0,mask=baseMask(r,c),idx=r*9+c,b=box(r,c);for(let bits=mask;bits;bits&=bits-1){const one=bits&-bits,d=digitForBit[one];grid[r][c]=d;rows[r]|=one;cols[c]|=one;boxes[b]|=one;let ok=true;for(const ci of cellClues[idx])if(!clueFeasible(ci)){ok=false;break;}rows[r]^=one;cols[c]^=one;boxes[b]^=one;grid[r][c]=0;if(ok)out|=one;}return out;}
  ...
  function visit(differs){
    ...
    while(changed&&!contradiction){
      ...
      for(let r=0;r<9&&!changed;r++)for(let c=0;c<9;c++)if(!grid[r][c]){
        const mask=baseMask(r,c),count=bitCount[mask];
        if(!count){contradiction=true;break;}
        if(count===1){const one=mask&-mask;put(r,c,one);let ok=true;for(const ci of cellClues[r*9+c])if(!clueFeasible(ci)){ok=false;break;}if(!ok){unput(r,c,one);contradiction=true;break;}forcedStack.push([r,c,one]);forced++;if(digitForBit[one]!==solution[r][c])differs=true;changed=true;break;}
      }
    }
    if(!contradiction){
      ...
      if(!contradiction){
        ...
        else{const knownBit=1<<(solution[br][bc]-1),nonKnown=bm&~knownBit;for(let bits=nonKnown;bits&&!found&&!exhausted;bits&=bits-1){const one=bits&-bits;put(br,bc,one);visit(true);unput(br,bc,one);}if(!found&&!exhausted&&(bm&knownBit)){put(br,bc,knownBit);visit(differs);unput(br,bc,knownBit);}}
      }
    }
    for(let i=forcedStack.length-1;i>=0;i--){const [r,c,one]=forcedStack[i];unput(r,c,one);}
  }
  visit(false);return{alternative:found,nodes,exhausted,forced};
}
...
for(const idx of remaining){const r=Math.floor(idx/9),c=idx%9,old=puzzle[r][c];puzzle[r][c]=0;const check=findAlternative(puzzle,9000);totalNodes+=check.nodes;totalForced+=check.forced;if(check.exhausted){puzzle[r][c]=old;unresolved++;continue;}if(check.alternative){puzzle[r][c]=old;continue;}removed++;}
const finalCheck=findAlternative(puzzle,30000);totalNodes+=finalCheck.nodes;totalForced+=finalCheck.forced;
const elapsed=performance.now()-t0;
if(finalCheck.exhausted||finalCheck.alternative)throw new Error('cheap propagation probe lost exact uniqueness');
const givens=puzzle.flat().filter(Boolean).length;
console.log(`LITTLE_KILLER_CHEAP_PROPAGATION baselineGivens=${remaining.length} finalGivens=${givens} removed=${removed} unresolved=${unresolved} nodes=${totalNodes} forced=${totalForced} runtimeMs=${elapsed.toFixed(1)}`);
console.log('LITTLE_KILLER_CHEAP_PROPAGATION PASS');
