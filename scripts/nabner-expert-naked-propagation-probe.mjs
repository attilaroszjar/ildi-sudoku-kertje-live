...
const refs=[...indexHtml.matchAll(/<script src="([^"]+)"><\/script>/g)].map(m=>m[1]);
...
const seed=Number(process.env.NABNER_NAKED_PROP_SEED||92003);
if(!Number.isSafeInteger(seed)||seed<0)throw new RangeError('NABNER_NAKED_PROP_SEED invalid');
...
function makeIndex(data){
  ...
  for(const rawLine of raw){const line=Array.isArray(rawLine)?rawLine:rawLine.cells||[];const id=lines.length;lines.push(line);for(const [r,c] of line)cells[r][c].push(id);}
  return{lines,cells};
}

function countNakedPropSolutions(source,data,limit,stats={}){
  ...
  for(let r=0;r<n;r++)for(let c=0;c<n;c++){
    ...
    for(const lineId of index.cells[r][c]){if(expandedForbidden(lineMasks[lineId],full)&bit)return 0;lineMasks[lineId]|=bit;}
  }
  function maskAt(r,c){
  ...
  function place(r,c,bit,trail){
    ...
    trail.push([r,c,bit]);stats.propagated++;
  }
  function rollback(trail){
    for(let i=trail.length-1;i>=0;i--){const [r,c,bit]=trail[i];for(const lineId of index.cells[r][c])lineMasks[lineId]^=bit;rows[r]^=bit;cols[c]^=bit;boxes[boxOf(r,c)]^=bit;grid[r][c]=0;}
  }
  function propagate(trail){
    while(true){
      ...
      for(let r=0;r<n;r++)for(let c=0;c<n;c++)if(!grid[r][c]){
        ...
        if(cnt===1){forced=[r,c,mask];break;}
      }
      if(!forced)return true;
      place(forced[0],forced[1],forced[2],trail);
    }
  }
  let found=0;
  function visit(){
    ...
    for(let r=0;r<n;r++)for(let c=0;c<n;c++)if(!grid[r][c]){
      const mask=maskAt(r,c),cnt=bitCount(mask);let pressure=index.cells[r][c].length*n;
    ...
    for(let bits=bm;bits&&found<limit;bits&=bits-1){
      const one=bits&-bits;grid[br][bc]=bitToDigit(one);rows[br]|=one;cols[bc]|=one;boxes[b]|=one;
...
for(const index of order){
  ...
  const stats={};const t0=performance.now();const solutions=countNakedPropSolutions(puzzle,baseline.data,2,stats);const elapsed=performance.now()-t0;
  totalMs+=elapsed;maxMs=Math.max(maxMs,elapsed);totalNodes+=stats.nodes;totalPropagated+=stats.propagated;
  const keep=solutions===1;if(keep)accepted++;else{rejected++;puzzle[r][c]=old;}
  attempts.push({index,cell:`r${r+1}c${c+1}`,accepted:keep,solutions,givensAfter:countClues(puzzle),runtimeMs:+elapsed.toFixed(1),nodes:stats.nodes,branches:stats.branches,deadEnds:stats.deadEnds,propagated:stats.propagated});
}
const finalStats={};const finalSolutions=countNakedPropSolutions(puzzle,baseline.data,2,finalStats);
...
console.log('NABNER_NAKED_PROPAGATION '+JSON.stringify({seed,baseGivens:30,acceptedRemovals:accepted,rejectedRemovals:rejected,finalGivens:countClues(puzzle),density:+(countClues(puzzle)/81).toFixed(3),finalSolutions,referenceSolutions,mismatches,essential,local,totalMs:+totalMs.toFixed(1),maxMs:+maxMs.toFixed(1),totalNodes,totalPropagated,finalNodes:finalStats.nodes,finalPropagated:finalStats.propagated,hottest}));
const pass=mismatches===0&&finalSolutions===1&&essential&&local&&accepted+rejected===30;
console.log('NABNER_NAKED_PROPAGATION_GATE:'+(pass?'PASS':'FAIL'));
