...
function loadRuntime(){
  ...
  const refs=[...index.matchAll(/<script src="([^"]+)"><\/script>/g)]
    .map(m=>m[1])
    .filter(x=>/^games\/(?:sudoku-bank(?:-iteration\d+)?|sudoku-generator|iteration13-generator-hardening)\.js$/.test(x));
...
const jobs=Array.from({length:samples},(_,i)=>({seed:(0x65730000+i*0x1f3d)>>>0}));
const rows=[];
...
console.log('EVEN_SANDWICH_PRODUCTION_GATE '+JSON.stringify({samples,perSampleTimeoutMs,concurrency,report}));
console.log('EVEN_SANDWICH_PRODUCTION_GATE:'+(pass?'PASS':'FAIL'));
