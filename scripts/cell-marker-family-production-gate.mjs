...
function topHeavySemanticTopology(generated){
  const solution=generated.solution||[];
  const active=[];
  for(let r=0;r<solution.length-1;r+=1){
    for(let c=0;c<(solution[r]||[]).length;c+=1){
      const upper=solution[r][c],lower=solution[r+1]&&solution[r+1][c];
      if(Number.isInteger(upper)&&Number.isInteger(lower)&&upper%2===lower%2)active.push([r,c]);
    }
  }
  return canonicalJson(active);
}

function topology(generated){
  if(generated&&generated.id==='top-heavy-parity')return topHeavySemanticTopology(generated);
...
async function runVariant(id){
  const jobs=Array.from({length:samples},(_,i)=>({id,seed:(0x434d0000+ids.indexOf(id)*0x10000+i*0x1f3d)>>>0}));
  ...
  return {id,pass:result.pass,report:{...result.summary,exhausted:exhausted.length,failureReasons:result.failures,failureSamples:rows.filter(r=>r.status!=='PASS').map(r=>({seed:r.seed,status:r.status,exact:r.exact,classic:r.classic,replayOk:r.replayOk,error:r.error}))}};
}

const results=[];
for(const id of ids)results.push(await runVariant(id));
const pass=results.every(r=>r.pass);
console.log('CELL_MARKER_FAMILY_PRODUCTION_GATE '+JSON.stringify({variants:ids.length,samplesPerVariant:samples,perSampleTimeoutMs,results}));
for(const r of results)console.log(`CELL_MARKER ${r.id} ${r.pass?'PASS':'FAIL'} solution=${r.report.solutionUnique}/${samples} structural=${r.report.structuralSolutionUnique}/${samples} topology=${r.report.topologyUnique}/${samples} puzzle=${r.report.puzzleUnique}/${samples} maxMs=${r.report.maxMs}`);
console.log('CELL_MARKER_FAMILY_PRODUCTION_GATE:'+(pass?'PASS':'FAIL'));
