...
async function runVariant(id){
  const jobs=Array.from({length:samples},(_,i)=>({id,seed:(0x4f520000+ids.indexOf(id)*0x10000+i*0x1f3d)>>>0}));
...
console.log('ORDERING_FAMILY_PRODUCTION_GATE '+JSON.stringify({variants:ids.length,samplesPerVariant:samples,perSampleTimeoutMs,results}));
for(const r of results){
  console.log(`ORDERING ${r.id} ${r.pass?'PASS':'FAIL'} solution=${r.report.solutionUnique}/${samples} structural=${r.report.structuralSolutionUnique}/${samples} topology=${r.report.topologyUnique}/${samples} puzzle=${r.report.puzzleUnique}/${samples} maxMs=${r.report.maxMs}`);
}
console.log('ORDERING_FAMILY_PRODUCTION_GATE:'+(pass?'PASS':'FAIL'));
