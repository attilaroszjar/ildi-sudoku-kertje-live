...
function count16ReverseRows(source,limit=2,stats){
  ...
  for(const idx of empties){
    ...
    const digits=[];
    while(mask){const bit=mask&-mask;mask^=bit;digits.push(32-Math.clz32(bit));}
    for(let di=digits.length-1;di>=0;di--){const d=digits[di];addRow([cellCol(r,c),rowDigitCol(r,d),colDigitCol(c,d),boxDigitCol(b,d)]);}
  }
  ...
  search();
  if(stats)Object.assign(stats,{nodes,branches,deadEnds,solutions,openCells:empties.length,rowOrder:'descending-digit'});
  return solutions;
}
...
const reversed=carve(count16ReverseRows);
const sameDecisions=baseline.decisions.length===reversed.decisions.length&&baseline.decisions.every((x,i)=>x===reversed.decisions[i]);
const samePuzzle=JSON.stringify(baseline.grid)===JSON.stringify(reversed.grid);
const worstReversed=reversed.calls.slice().sort((a,b)=>b.ms-a.ms).slice(0,10);
emit('SUDOKU16_ROW_ORDER_DIFFERENTIAL',{
  ...
  descending:{ms:rounded(reversed.ms),finalGivens:countGivens(reversed.grid),accepted:reversed.accepted,rejected:reversed.rejected,verificationMs:rounded(reversed.verificationMs),finalStats:reversed.finalStats,worstCalls:worstReversed.map(x=>({...x,ms:rounded(x.ms)}))},
  sameDecisions,samePuzzle,speedup:rounded(baseline.ms/reversed.ms)
});
if(!baseline.unique||!reversed.unique||!sameDecisions||!samePuzzle)throw new Error('row-order differential mismatch');
console.log('SUDOKU16_ROW_ORDER_DIFFERENTIAL:PASS');
