...
function loadRuntime(){
  ...
  const G=ctx.SudokuGenerator,variant=ctx.SudokuBank&&ctx.SudokuBank.find(v=>v.id==='next-to-nine');
  if(!G||typeof G.make!=='function')throw new Error('SudokuGenerator.make missing');
  if(!variant)throw new Error('Next to Nine missing');
  return {G,variant};
}
...
for(let i=0;i<samplesPerDifficulty;i++){
  const seed=(0x6e743900+i*0x1f3d)>>>0;
...
console.log('NEXT_TO_NINE_DIFFICULTY_GATE '+JSON.stringify({samplesPerDifficulty,timeoutMs,concurrency,byDifficulty,orderingBasis:'paired-givens-pressure',pairedClueOrder,failures:rows.filter(r=>r.status!=='PASS')}));
console.log('NEXT_TO_NINE_DIFFICULTY_GATE:'+(pass?'PASS':'FAIL'));
