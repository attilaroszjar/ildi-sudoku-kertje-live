(function(root){
  ...
  function searchStats(source,variant){
    ...
    function extraValid(){
      ...
      for(var i=0;i<clues.length;i++){var cl=clues[i],line=orientedLine(grid,cl);if(line.every(Boolean)&&visibleCount(line)!==cl.count)return false;}
      var dominoes=(variant.data&&variant.data.dominoes)||[],target=null;
      for(i=0;i<dominoes.length;i++){
        var dm=dominoes[i],a=grid[dm[0][0]][dm[0][1]],b=grid[dm[1][0]][dm[1][1]];
        if(a&&b){var sum=a+b;if(target===null)target=sum;else if(sum!==target)return false;}
      }
      return true;
    }
