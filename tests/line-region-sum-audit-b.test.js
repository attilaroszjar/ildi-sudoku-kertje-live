...
function validateSegments(p){for(const line of p.data.lines){const segs=rs.sums(p.solution,line);assert.ok(segs.length>=2);assert.ok(segs.every(s=>s.cells.length>=2&&s.complete));assert.equal(new Set(segs.map(s=>s.sum)).size,1);}}

test('region-sum audit B: 32x3 exact/essential/diverse with ordered measured difficulty and bounded runtime',()=>{
  const solutions=new Set(),tops=new Set(),scores={gentle:[],focused:[],expert:[]},times={gentle:[],focused:[],expert:[]};
  for(let i=0;i<32;i++){
    const seed=0x525300+i*29;
    for(const level of levels){
      ...
      assert.equal(p.generation.unique,true);assert.equal(p.generation.variantEssential,true);validateSegments(p);
  ...
  console.log(`REGION_SUM_B PASS total=96 structuralSolution=${solutions.size}/32 structuralTopology=${tops.size}/32 difficulty=${med[0]}<${med[1]}<${med[2]} p95Ms=${runtimes.map(x=>x.toFixed(1)).join('/')}`);
});

test('region-sum audit B confirms production wiring',()=>{
  const src=fs.readFileSync(path.join(root,'games/line-generator-region-sum-hardening.js'),'utf8');
  assert.match(src,/LineGeneratorRegionSum/);assert.match(html,/line-generator-region-sum\.js/);
});
