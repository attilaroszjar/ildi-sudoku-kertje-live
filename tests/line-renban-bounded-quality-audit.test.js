...
function assertRenban(ctx,g){for(const line of g.data.lines){assert.equal(ctx.LineGeneratorCore.validateSimplePath(line,{minLength:4,maxLength:6}),true);const values=line.map(([r,c])=>g.solution[r][c]),unique=new Set(values);assert.equal(unique.size,line.length);assert.equal(Math.max(...values)-Math.min(...values)+1,line.length);}}
...
test('Renban bounded quality audit 32 seeds x 3 difficulties',()=>{
  ...
  for(const seed of SEEDS){
    ...
    for(const difficulty of DIFFICULTIES){
      ...
      assert.equal(a.generation.unique,true);assert.equal(a.generation.variantEssential,true);assert.equal(a.generation.generatorFamily,'line-renban-fresh-fill-mrv');assert.equal(a.generation.pilot,false);
      assert.equal(ctx.SudokuGenerator.countVariantSolutions(a.puzzle,a,2),1);assert.ok(ctx.SudokuGenerator.countSolutions(a.puzzle,2)>1);assertRenban(ctx,a);
  ...
  console.log('RENBAN_GENERATOR_AUDIT '+JSON.stringify(summary));
